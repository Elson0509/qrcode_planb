...
const VisitorEdit = props => {
    ...
    useEffect(()=>{
      api.get(`api/condo/${props.route.params.user.condo_id}`)
        ...
        .catch(err=>{
          Toast.show(err.response?.data?.message || 'Um erro ocorreu. Tente mais tarde. (TE1)', Constants.configToast)
        })
        .finally(()=>{
          setLoading(false)
        })
    ...
    const confirmHandler = _ =>{
      ...
      .then(res=>{
        ...
        .catch(err2=>{
          Toast.show(err2.response?.data?.message || 'Um erro ocorreu. Tente mais tarde. (TE2)', Constants.configToast)
        })
      })
      .catch(err=>{
        Toast.show(err.response?.data?.message || 'Um erro ocorreu. Tente mais tarde. (TE3)', Constants.configToast)
      })
      .finally(()=>{
        setLoading(false)
