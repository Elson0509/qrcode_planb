...
import dummyListVisitors from '../../../dummyListVisitors.json'
...
const ThirdList = props => {
    ...
    useEffect(()=>{
      setUsers(dummyListVisitors.data)
    }, [])

    const delUnitModal = unit => {
      setUnitSelected(unit)
      setMessage(`Excluir visitantes do Bloco ${unit.bloco} unidade ${unit.unidade}?`)
      setModal(true)
    }
    ...
    const editHandler = unitId => {
      props.navigation.navigate('ThirdEdit', {id: unitId})
    }
                    ...
                    >
                      ...
                      <View style={{justifyContent: 'space-between', flexDirection: 'row'}}>
                        <View style={{maxWidth: 250}}>
                          <View>
                            <Text style={styles.subTitle}>Terceirizados:</Text>
                            {
                              obj.item.visitors.map((res, ind)=>{
                                return (
